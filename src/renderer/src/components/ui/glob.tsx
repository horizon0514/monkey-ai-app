...
/**
 * https://www.vapiblocks.com/components/glob
 * @param param0
 * @returns
 */
const AbstractBall: React.FC<any> = ({
  ...
  const pointRef = useRef<Mesh | null>(null)
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      vertex: document.getElementById('noiseVertexShader')!.textContent!,
    ...
    const mesh = new Mesh(gl, { geometry: sphere, program })
    const point = new Mesh(gl, { geometry: sphere, program, mode: gl.POINTS })

    mesh.setParent(scene)
    point.setParent(scene)
    ...
    const animate = () => {
      ...
      mesh.visible = !spherePoints && !sphereWireframe
      point.visible = spherePoints || sphereWireframe
    ...
    return () => {
      ...
      if (container && gl && gl.canvas.parentElement === container) {
  ...
  useEffect(() => {
    if (cameraRef.current) {
      const pos = cameraRef.current.position as any
      gsap.to(pos, {
        duration: 2,
        z: 300 - cameraZoom
      })
