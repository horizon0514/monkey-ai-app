...
export const SettingsModal: React.FC = () => {
  ...
  useEffect(() => {
    window.electron.getSiteConfigs().then((configs: SiteConfig[]) => {
      setSites(defaultSites.map(site => ({ ...site, enabled: configs.some(config => config.id === site.id && config.enabled) })));
    });
  }, []);
  ...
  return (
    <div className="w-full h-full bg-background flex flex-col">
      <div className="flex h-12 items-center border-b border-border/40 drag-region bg-background shrink-0">
        <div className="flex-1 flex items-center justify-center">
          <span className="text-sm font-medium">设置</span>
        </div>
        {!isMacOS && <div className="w-[78px]" />}
      </div>
      <div className="flex-1 min-h-0 overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border/40 [&::-webkit-scrollbar-track]:bg-transparent">
